        ...
        <div class="card-form">
            ...
            <form action="" method="post">
                <div class="card-form__inner">
                    ...
                    <?php if(isset($_GET['oprice'])) {?>
                    <button class="card-form__button" type="submit" name="payNowo">&#8377;
                        <?php echo $_GET['oprice']; ?>
                    </button>
                    <?php }else{ ?>
...
if (isset($_REQUEST['payNow'])) {
    ...
    if ($id > 0) {
        ...
        $qry = "UPDATE tbl_learners_details SET payment_status='Paid' WHERE user_id=?";
        $stmt = $conn->prepare($qry);
        $stmt->bind_param("i", $id);

        if ($stmt->execute()) {
            echo "<script>alert('Successfully Paid'); window.location = 'newlicense.php?flag=2';</script>";
        } else {
...
if (isset($_POST['payNowo'])) {
    // Sanitize and validate input
    $id = $_SESSION['id'];
    $paid='paid';
    $qry = "UPDATE tbl_services SET payment_status= ? WHERE user_id=?";
        $stmt = $conn->prepare($qry);
...
$price = ''; // If this is not used, you can remove it.
?>
